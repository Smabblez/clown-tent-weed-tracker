(() => {
  ...
  function openCorrection(type, id) {
    ...
    const record = (isGrow ? state.grows : isSupply ? state.supplies : state.sales).find(function (row) { return String(row.id) === String(id); });
    ...
    setCorrectionGroup($("[data-correction-supply]"), isSupply);
    setCorrectionGroup($("[data-correction-sale]"), !isGrow && !isSupply);
    $("[data-correction-title]").textContent = isGrow ? "Correct grow entry" : isSupply ? "Correct supply purchase" : "Correct sale";
    if (isGrow) {
      ...
      form.supplyDate.value = dateInputValue(record.timestamp);
      setRecordSelectValue(form.supplyBuyer, record.buyer);
      form.supplyItem.value = record.item || "";
      form.supplyQuantity.value = num(record.quantity);
      form.supplyUnitCost.value = num(record.unitCost);
      form.supplyNotes.value = record.notes || "";
    } else {
