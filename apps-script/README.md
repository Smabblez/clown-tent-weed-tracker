...
Payout balances are visible to normal tracker users, but `settleSale`, `settleSupply`, and the manager-only `settlePayouts` batch action require the private `ADMIN_CODE` before any payout can be marked paid. Supply purchases add a separate reimbursement payable to the `buyer`; their `paidAt` timestamp is independent from supply-cost recovery through the gang share. The batch action accepts only sale IDs with `grower`/`seller` roles or supply IDs with the `supply` role, updates only settlement timestamps under the document lock, and returns a fresh bootstrap snapshot.

`updateGrow`, `updateSupply`, `updateSale`, `deleteGrow`, `deleteSupply`, `deleteSale`, and `reopenPayout` also require `ADMIN_CODE`. They validate and write an immutable before/after record with the manager's reason to Web_Corrections. Payout-affecting sale corrections clear both paid timestamps, and supply corrections that change the buyer, quantity, or cost clear `paidAt` so the corrected reimbursement must be settled again. Deletions set `deletedAt` and `deleteReason`; deleted records remain in the raw sheet for auditing but are excluded from inventory, sales, supply recovery, and payout totals.
